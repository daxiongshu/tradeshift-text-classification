...
def data(path, label_path=None):
    for t, line in enumerate(open(path)):
        ...
        for m, feat in enumerate(line.rstrip().split(',')):
            if m == 0:
                ID = int(feat)
            elif m in [23,22,113,114,53,54,138,139,39,38,37,36,6,7,8,9,40,120,121,122,123,124,125]:
...
def update2(alpha, w, n, x, p, y):
    ...
    for c,i in enumerate(x[146+hh:]):
        ...
        # note that in our case, if i in x then x[i] = 1.
        n[D+c] += 1.0
        w[D+c] -= ((p - y) * i * alpha/(10+n[D+c]/200000)  +0*w[D+c])  # /2 is good!
...
with open(sub_dir+'./submissiontk20.csv', 'w') as outfile:
